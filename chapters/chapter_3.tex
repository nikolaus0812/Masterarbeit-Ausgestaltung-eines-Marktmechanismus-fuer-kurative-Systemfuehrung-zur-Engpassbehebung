...
Die vorliegende Arbeit ergänzt das Engpassmanagement um eine Systemdienstleistung, deren Gegenstand nicht die Leistungsanpassung selbst ist, sondern die Bereitschaft dazu im Fehlerfall.
...
%In diesem Kapitel werden die Anforderungen an einen solchen Mechanismus abgeleitet, die kurative Reservierung als Produkt entworfen und der kurative Reservierungspreis mit einem Optimierungsmodell aus der Sicht des \ac{EIV} bestimmt.
In diesem Kapitel werden die Anforderungen an einen solchen Mechanismus abgeleitet, die kurative Reservierung als Produkt entworfen und der kurative Reservierungspreis mit einem Optimierungsmodell aus der Sicht des Betreibers bestimmt.
...
% ENTFERNT am 22.09.2026, Entscheidung des Verfassers zu G1: der geparkte
% Unterabschnitt zum Abgleich bestehender Marktdesignansaetze wandert in die
% Diskussion in Kapitel 5. Damit sind die beiden hier verbliebenen Marken
% ohne Ueberschrift gegenstandslos. Entfernt wurden:
%\label{sec:market_design_comparison}
%\label{sec:product_design}
% Der einzige Verweis auf sec:market_design_comparison stand auskommentiert
% in chapter_2.tex Zeile 1451 und bleibt auskommentiert.
...
Angenommen ist der ungünstigste Fall eines einstündigen Abrufs, der über zwei Zeitscheiben reichen kann.
...
Die zugesagte Leistung einer solchen Anlage ist dabei der Anteil der installierten Leistung, der über die Bindungsdauer mit hoher Wahrscheinlichkeit verfügbar ist.
...
Die Zeitscheibe von einer Stunde ist zugleich die Bindungsdauer und entspricht der Dauer des ungünstigsten Abrufs.
...
Den Bedarf ermitteln die \ac{ÜNB} in ihren Rechenläufen zur Planung des Engpassmanagements am Vortag, sodass er vor dem Zuschlag feststeht.
...
Als Netzknoten kommen zunächst die Knoten des 380-kV-Netzes in Betracht, später auch die 110-kV-Knoten.
...
Der kurative Reservierungspreis ist aus dem Betrieb des Speichers selbst zu bestimmen.
...
Die Variablen stehen vollständig in Anhang~\ref{anh:modell}.
...
% ERGAENZT am 22.09.2026, Vorgabe des Verfassers: die Folge der Sperrung fuer
% den Umfang der Auswertung gehoert hierher und nicht in die Einleitung von
% Kapitel 4.
Über das Jahr 2025 bleiben damit 8758 der 8760 Stunden je Richtung, für die das Modell einen Preis bestimmt.
...
Was der Betreiber damit zusätzlich verdient, ließe sich nur aus den Orderbüchern ablesen und bleibt ohne sie eine Schätzung.
Jede zusätzliche Handelsmöglichkeit verbessert den erzielbaren Preis, sodass der \ac{IDC} insgesamt mehr trägt als ein einzelnes Geschäft zum Index.
...
Wie viel Arbeit die Anlage liefern darf, prüft eine Sensitivität.
Sie stellt den anteiligen Abruf gegen eine freie Lieferung bis zur eigenen Reservierung und bis zu zehn Prozent des deutschen Abrufs.
...
\end{equation}
Die Mindestgröße von 25\,\si{MW} ist ein Parameter des Produkts, sodass eine Stunde entweder ungebunden bleibt oder mit mindestens diesem Wert reserviert wird, wie in Abschnitt~\ref{sec:model_scope} beschrieben.
...
Die Suche setzt dabei voraus, dass der Füllgrad einer Stunde mit ihrem vorgegebenen Reservierungspreis steigt, sodass es je Stunde genau einen solchen Preis gibt.
Ein Lauf in der Mitte eines Preisintervalls sagt deshalb, ob der gesuchte Preis darüber oder darunter liegt, und halbiert das Intervall, was mit wenigen Läufen auskommt und kein feines Preisgitter braucht.
Die Suche läuft in zwei gleich aufgebauten Iterationen, wie Abbildung~\ref{fig:reservierungspreis_ablauf} zeigt und wie Anhang~\ref{anh:ablauf} mit allen Schritten dargestellt ist.

\begin{figure}[H]
    \centering
% UMGESTELLT am 22.09.2026 nach UPDATE_03 des Analyse-Repositorys: die Datei
    % wird jetzt in voller Breite und flacher geliefert (453,5 mal 226,8 statt
    % 371,9 mal 275,2 pt). Sie steht damit buendig im Satzspiegel und laesst
    % weiterhin Text unter sich. Alte Fassung:
    %    \includegraphics[width=0.82\textwidth]{figures/chapter_3/reservierungspreis_bisektion.pdf}
    \includegraphics[width=\textwidth]{figures/chapter_3/reservierungspreis_bisektion.pdf}
    \caption{Bisektion für eine Beispielstunde, (a) bei festem Preis der vollen Reservierung und (b) bei einem Preis, der während der Suche steigt. Die Preisachse ist logarithmisch.}
...
\begin{figure}[t]
    ...
    \caption{Bestimmung des kurativen Reservierungspreises in zwei Iterationen, Symbole nach DIN 66001. Der gestrichelte Kasten fasst die Bisektion zusammen.}
...
Abgerufen wird der Speicher, sobald der Sollwert des \ac{ÜNB} die Leistung aller günstigeren Gebote übersteigt, geliefert wird der Sollwert bis zur vorgehaltenen Leistung.
...
% ABBILDUNGSLIEFERUNG am 21.09.2026, Anweisung aus bess_dispatch_optimization (ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md, Stand 21.09.2026 16:14), Abschnitt 8: Kennzahlen des aktuellen Gesamtlaufs vom 17. bis 19.09.2026. Alte Fassung:
%Für das Jahr 2025 weist der Revenue-Index marktübergreifend 259,7 Tausend Euro je Megawatt und Jahr aus.
Für das Jahr 2025 weist der Revenue-Index marktübergreifend 260,0 Tausend Euro je Megawatt und Jahr aus.
...
Der Vergleich beginnt mit den Einzelmärkten, die im Einzelnen in Anhang~\ref{anh:validierung} dargestellt sind.
...
In der \ac{aFRR} erreicht das Optimierungsmodell das 1,51-Fache des Revenue-Index, was vor allem an der Bemessung der Leistung liegt.
...
Auch die Liefermenge der \ac{aFRR}-Arbeit hängt auf beiden Seiten an der vorgehaltenen Leistung, folgt aber den Regeln, die Abschnitt~\ref{sec:validation_benchmark} und Abschnitt~\ref{sec:input_data} je Seite nennen.
...
% AKTUALISIERT am 22.09.2026 nach ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md Abschnitt 7: der Intraday-Handel ist in den ID1 (0,97-Faches) und den IDC (0,89-Faches) aufgeteilt, bisher stand dort ein einzelner Wert. Alte Fassung, nicht wieder aufzunehmen:
%In den Energiemärkten bleibt das Optimierungsmodell unter dem Revenue-Index, am \ac{DA} mit dem 0,85-Fachen und im Intraday-Handel mit dem 0,89-Fachen.
In den Energiemärkten bleibt das Optimierungsmodell unter dem Revenue-Index, am \ac{DA} mit dem 0,85-Fachen, am \ac{ID1} mit dem 0,97-Fachen und am \ac{IDC} mit dem 0,89-Fachen.
...
% ABBILDUNGSLIEFERUNG am 21.09.2026, Anweisung aus bess_dispatch_optimization (ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md, Stand 21.09.2026 16:14), Abschnitt 8: Kennzahlen des aktuellen Gesamtlaufs vom 17. bis 19.09.2026. Alte Fassung:
%Das Optimierungsmodell liegt im Mittel bei 348,3 Tausend Euro je Megawatt und Jahr.
% ERGAENZT am 22.09.2026 nach ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md Abschnitt 7: der Revenue-Index ist mit 260,0 beziffert, damit sich das Verhaeltnis von 1,31 nachrechnen laesst. Alte Fassung:
%Das Optimierungsmodell liegt im Mittel bei 340,7 Tausend Euro je Megawatt und Jahr.
Das Optimierungsmodell liegt im Mittel bei 340,7 und der Revenue-Index bei 260,0 Tausend Euro je Megawatt und Jahr.
% ABBILDUNGSLIEFERUNG am 21.09.2026, Anweisung aus bess_dispatch_optimization (ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md, Stand 21.09.2026 16:14), Abschnitt 8: Kennzahlen des aktuellen Gesamtlaufs vom 17. bis 19.09.2026. Alte Fassung:
...
Das Verhältnis beider Reihen beträgt im Median 1,31 bei einer Spanne von 1,04 bis 1,50, und das Optimierungsmodell liegt in allen zwölf Monaten über dem Revenue-Index.
% ABBILDUNGSLIEFERUNG am 21.09.2026, Anweisung aus bess_dispatch_optimization (ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md, Stand 21.09.2026 16:14), Abschnitt 8: Kennzahlen des aktuellen Gesamtlaufs vom 17. bis 19.09.2026. Alte Fassung:
...
Die beiden Reihen korrelieren mit einem Koeffizienten von 0,974, sodass das Optimierungsmodell den Jahresgang des Revenue-Index abbildet.
...
% ABBILDUNGSLIEFERUNG am 21.09.2026, Anweisung aus bess_dispatch_optimization (ANWEISUNG_LIEFERUNG_ABBILDUNGEN.md, Stand 21.09.2026 16:14), Abschnitt 2: die Abbildung ohne Mindestgroesse entfaellt ersatzlos, weil die Schranke am 11.02.2025 bei 5 Euro je Megawatt und Stunde nach der Korrektur der Kausalitaetsbedingung nicht bindet (kleinste Reservierung 45,08 MW). Der Satz und der Verweis auf den gestrichenen Anhang sind gestrichen, nicht wieder aufzunehmen:
...
%Dieser Fahrplan ohne die Mindestgröße steht in Anhang~\ref{anh:dispatch_ohne}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figures/chapter_3/dispatch_festpreis_2025-02-11_p5.pdf}
    \caption{Fahrplan und Ladezustand am 11.02.2025 bei einem vorgegebenen Reservierungspreis von 5\,\EurMWh{} und der Mindestgröße von 25\,\si{MW}, oben positiv, unten negativ.}
